...
# date: May 11, 2023
# file: calculatorGUI.py , calculator function displaying the calculator using Tkinter module and GUI
# input:  GUI mainloop , expression
...
def calculator(gui):   
    ...
    b_float = addButton(gui,entrybox,'.')
    b_leftbracket = addButton(gui,entrybox, "(")
    b_rightbracket = addButton(gui,entrybox, ")")
    # add buttons to the grid
    buttons =[ b7,    b8, b9,    b_clr, 
               b4,    b5, b6,    b_sub, 
               b1,    b2, b3,    b_add, 
               b_mult,b0, b_div, b_eq, b_pow,b_float,b_leftbracket ,b_rightbracket  ]
    k = 4           
    for i in range(k):
        for j in range(k):
            buttons[i*k+j].grid(row=i+1, column=j, columnspan=1)
    buttons[16].grid(row=5,column=3,columnspan=1)                   #buttons for new operators ^, . , (, )
    buttons[17].grid(row=5,column=2,columnspan=1)
    buttons[18].grid(row=5,column=0,columnspan=1)
    buttons[19].grid(row=5,column=1,columnspan=1)
...
def clickButton(entrybox, value):
    if value == '=':
        ...
        calculation = calc.calculate(equation)
        entrybox.delete(0, END)
        entrybox.insert(END, calculation)
    elif value == 'c':
        # Clear the entry box when C button is clicked
        entrybox.delete(0, END)
    else:
        # Append the clicked button value to the entry box
